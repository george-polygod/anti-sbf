...
contract AntiSBFToken is IERC20Upgradeable, OwnableUpgradeable {
    ...
    function initialize(
        ...
        antiBot = BPContract(address(antiBot_));
        antiBot_.addPair(uniswapV2Pair);
        enableAntiBot = useAntiBot_;
    ...
    function setEnableAntiBot(bool _enable) external onlyOwner {
        require(address(antiBot) != address(0),"antiBot not availabe");
    ...
    function _transfer(
        ...
        if (enableAntiBot) {
                antiBot.protect(from, to, amount);
        }
        ...
        if (!_isExcludedFromFee[from] && to == address(uniswapV2Pair)) {
            takeFee = true;
        }
